...
class State:
    ...
    def calculate_costs_for_all(self):
        ...
        for student_name, team_number in self.student_to_team_map.iteritems():
            ...
            student_cost = student.calculate_total_cost(team)
            if student_cost > 0:
                self.student_cost_queue.put((-1 * student_cost, student_name))  # Making it Max Heap
    ...
    def get_student_team_members(self, student_name):
    ...
    def cost_of_placing_student_in_team(self, student_name, team_number):
        ...
        if student.pref_team_size == 0 or curr_team_size + 1 == student.pref_team_size:
    ...
    def assign_student_to_team(self, student_name, next_team_num):
        curr_team_num = self.student_to_team_map[student_name]
        self.student_to_team_map[student_name] = next_team_num
        self.team_to_student_map[curr_team_num].remove(student_name)
        self.team_to_student_map[next_team_num].add(student_name)
        # if len(self.team_to_student_map[next_team_num]) >= 3:
        #     print 'Gotcha: ', self.team_to_student_map[next_team_num]
...
def find_best_team_for_student(student_name, state):
    global student_dict
    best_cost = student_dict[student_name].calculate_total_cost(state.get_student_team_members(student_name))
    best_team_num = 0
...
def find_next_state(curr_state):
    while curr_state.student_cost_queue.qsize() > 0:
        student_name = curr_state.student_cost_queue.get()[1]
        best_team_num = find_best_team_for_student(student_name, curr_state)
        curr_state.assign_student_to_team(student_name, best_team_num)
        # print 'Printing intermediate states:'
        # print curr_state
    student_to_team_map = curr_state.student_to_team_map
    return State(student_to_team_map)


def find_best_state(curr_state):
...
def main():
    ...
    # Testing
    # curr_student_to_team_map = dict()
    # curr_student_to_team_map['djcran'] = 1
    # curr_student_to_team_map['steflee'] = 1
    # curr_student_to_team_map['fan6'] = 2
    # curr_student_to_team_map['kapadia'] = 2
    # curr_student_to_team_map['chen464'] = 3
    # curr_student_to_team_map['zehzhang'] = 3
    # best_state = find_best_state(State(curr_student_to_team_map))
    best_state = find_best_state(initialize())
    print '\nPrinting best state\n'
    print best_state
